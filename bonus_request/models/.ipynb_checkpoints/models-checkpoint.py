...
class Contract(models.Model):
    ...
    refundable_bonus = fields.Float(string="Bonus Amount")
    ...
    def _compute_day_value(self):
        for rec in self:
            if rec.employee_id and rec.contract_id:
                rec.day_value = rec.contract_id.new_wage / 26
            else:
                rec.day_value = 0.0
    ...
    def accounting_confirm(self):
        for rec in self:
            ...
            if res_user.has_group('account.group_account_manager'):
                rec.refundable_bonus = rec.bonus_amount * 0.10
                rec.employee_id.contract_id.refundable_bonus += rec.refundable_bonus
                rec.state = 'confirm'
            else:
